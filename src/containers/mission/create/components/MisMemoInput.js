...
const MisMemoInput = ({misMemo, setMisMemo, setMemoSaveBtnDisabled}) => {
  var misMemoLength = misMemo.length;
  const maxLengthOfMisMemo = 14;

  //메모의 길이가 0보다 길 면 memosave버튼 비활성화 상태값을  false로, 아무것도 입력되지 않은 경우 true로 설정합니다.
  useEffect(() => {
    misMemoLength > 0
      ? setMemoSaveBtnDisabled(false)
      : setMemoSaveBtnDisabled(true);
  }, [misMemo]);
